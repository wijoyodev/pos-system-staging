...
<head>
    <meta charset="UTF-8">
    <title>Faktur Pembelian - {{ $purchaseOrder->po_number }}</title>
    <style>
...
<body>
    ...
    <div class="header text-center">
        <h2 style="margin:0 0 2px;">{{ $purchaseOrder->store->name ?? 'Toko' }}</h2>
        <div style="font-size:10px;">{{ $purchaseOrder->store->address ?? '' }}</div>
        <h1 style="margin:8px 0 2px;font-size:16px;">FAKTUR PEMBELIAN</h1>
        <div style="font-size:12px;font-weight:bold;">{{ $purchaseOrder->po_number }}</div>
    </div>

    <div class="divider"></div>

    <table>
        <tr><td style="width:25%;">Supplier</td><td>: {{ $purchaseOrder->supplier->name ?? '-' }}</td></tr>
        <tr><td>Tanggal</td><td>: {{ $purchaseOrder->order_date->format('d/m/Y') }}</td></tr>
        <tr><td>Status</td><td>: {{ ucfirst($purchaseOrder->status) }}</td></tr>
    </table>
    ...
    <div class="stamp-box">
        <div class="stamp-item">
            <div style="font-size:9px;font-weight:bold;">Pemesan</div>
